clear
gcp;
regfile = 'D:\SC-Master\6.0-∞ﬂ¬Ì”„“»µ∫œÓƒø\1.0-Image-Processes\tmp\';
Files = dir([regfile,'*-NoRMC.tif']);
% Files = dir(regfile);
...
% options_rigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'bin_width',100,'max_shift',12,'us_fac',50,'init_batch',80);
% 
...
%     tic; [M1,shifts1,template1,options_rigid] = normcorre(Y,options_rigid); toc
    ...
    options_nonrigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'grid_size',[32,32],'mot_uf',4,'bin_width',50,'max_shift',12,'max_dev',8,'us_fac',50,'init_batch',50);
tic; [M2,shifts2,template2,options_nonrigid] = normcorre_batch(Y,options_nonrigid); toc
write_stack(uint16(M2),Files(i).folder,[Name,'-NoRMC'],16); 
end
% % % % %% compute metrics
% % % % 
